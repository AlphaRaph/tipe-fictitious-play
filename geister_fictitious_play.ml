(* Méthode Fictitious Play associée à IMP-Minimax pour résoudre le jeu 
Fantômes contre fantômes.
Seules des stratégies pures sont utilisées dans ce code.
BELLIOT Raphaël
2024-2025
v1.0 *)
...
(* Pour jouer un coup, il faut connaitre les coups précédents (game) et l'état 
actuel du jeu (info_set).
Représente une stratégie *)
type strategy = game -> info_set -> alternative     
...
type set =  | Set of (node * strategy * game) list             
            | PI_Set of (node * strategy) list * game
            | Leaf of node * strategy * game 
...
module Config = struct
    ...
    let configuration_4x4 = (4, 4, 1, 1, [13;14], [1;2], [0;3], [12;15], 
        [4; 1; -4; -1])
    let configuration_3x4 = (4, 3, 1, 1, [9;10], [1;2], [0;4], [8; 11], 
        [4; 1; -4; -1])
...
module Tools = struct 
    let list_to_array (l : 'a list) : 'a array =
        (*  sortie : tableau contenant les mêmes éléments que l dans le même 
                ordre *)
        if l = [] then [||] else
    ...
    let read_char (c : char) : (player option) * ghost * int = 
        ...
        match c with
        | '.' -> None, EmptyCase, -1 (* Le joueur renvoyé pour cette option n'a 
            pas d'importance *)
    ...
    let print_games (games : alternative list list) = 
        print_list games (fun game -> print_list game (fun (k_from, k_to)-> 
            Printf.printf "(%d, %d)" k_from k_to); print_newline ())
...
module Placement = struct
    ...
    Généralement, le fantôme d'indice i dans la liste correspond au fantôme 
    inconnu à placer sur la case i-ème case de départ, ou provenant de la i-ème 
    case de départ.*) 
    ...
    let () = Random.self_init () (* Initialisation du module Random, autrement 
                                    il donne toujours les mêmes valeurs au fil 
                                    des éxécutions.*)
    ...
    let possible_list_placements (number_of_blue_ghosts, number_of_red_ghosts : 
        int * int): list_placement list =
        (* post-condition : Une liste contenant tous les dispositions possibles 
            à tant de fantômes sous forme de listes.*)        
        let rec aux (i : int) (l_p : list_placement) (blues : int) (reds : int) 
            (acc : list_placement list ): list_placement list = 
            (* précondition : - -1 <= i < number_of_blue_ghosts + 
                number_of_red_ghosts *)
            if i = -1 then 
                l_p::acc 
            else
    ...
    let possible_placements (number_of_blue_ghosts, number_of_red_ghosts : 
        int * int): placement list = 
        (* post-condition : Une liste contenant tous les dispositions possibles
            à tant de fantômes. *)
        List.map list_placement_to_placement 
                (possible_list_placements 
                    (number_of_blue_ghosts,number_of_red_ghosts))
        
    let init_possible_placements = 
        possible_placements (Config.number_of_blue_ghosts, 
                             Config.number_of_red_ghosts)

    let init_possible_placements_array = (* Attention ! C'est un tableau ! 
                                (pour pouvoir y accéder en O(1) dans descend)*)
        Tools.list_to_array (init_possible_placements)

    let number_of_init_possible_placements = 
        Array.length init_possible_placements_array

    let add_init_placement ((p, b, b1, r1, b2, r2) : node) (pl : placement) : 
            node = 
        match p with 
        | P1 -> 
            P2,
    ...
    let put_placement ((p, b, b1, r1, b2, r2) : info_set) (pl : placement) : 
            node = 
        (* sortie : un noeud de l'ensemble d'information donné où les fantômes 
            inconnus sont colorés selon pl 
        Remarque : ne fonctionne pas correctement s'il y a un fantôme inconnu 
            de mort. *)
        p,
        (
        String.init 
            ...
            (fun k -> 
                ...
                | Some opp, _, cnt when opp <> p -> 
                    (* C'est les fantômes adverses qu'on colorie *)
                    Tools.char (Tools.other p) (pl cnt) cnt 
                | _ -> b.[k]  )
        ),
        b1, 
        r1, 
    ...
    let is_placement_node (p, b, b1, r1, b2, r2 : node) : bool = 
        (* Sortie : true ssi le noeud donné argument est la racine ou un fils 
            de la racine, i.e, un noeud où les alternatives sont des placements 
            et non des mouvements de fantôme.*)
        (b1 = 0 && r1 = 0) || (b2 = 0 && r2 = 0)


    let print_placement (n : node) (pl : placement) : unit = 
        let (p, b, b1, r1, b2, r2) = add_init_placement n pl in (* on ajoute le 
            placement mais il pense donc que c'est au joueur suivant de jouer *)
        for i = (Config.board_height-1) downto 0 do
            Printf.printf "|";
            for j = 0 to (Config.board_width-1) do
                match Tools.read_char b.[i*Config.board_width+j] with 
                | Some p', _, _ when p' = p -> Printf.printf " ? " (* c'est 
                                            donc bien les fantômes adverses *)
                | _ -> Printf.printf " %c " b.[i*Config.board_width+j]
            done;
            Printf.printf "|\n"
        done;
...
module GameTree = struct 
    ...
    let descend (k_from, k_to : alternative) 
                ((p, b, b1, r1, b2, r2) as n : node) : node = 
        (* sortie : renvoie le noeud dérivant du noeud n en jouant a *)
        if k_from < 0 then 
            Placement.add_init_placement n 
                Placement.init_possible_placements_array.(-k_from-1)
        else if k_to < 0 then 
            Placement.add_init_placement n 
                Placement.init_possible_placements_array.(
                    Random.int Placement.number_of_init_possible_placements)
        else 
            let b1', r1', b2', r2' = 
            ...
            String.init 
                Config.board_length 
                (fun k -> if k = k_from then '.' else if k = k_to then 
                    b.[k_from] else b.[k]),
            b1',
            r1',
            b2', 
    ...
    (* Début de partie où les deux joueurs ne connaissent pas la disposition 
    des fantômes adverses.*)
    ...
    let alternatives (x_set : pi_set) : alternative list = 
        (* sortie : la liste des coups possibles depuis un ensemble 
            d'information partielle.*)

        let (p, b, b1, r1, b2, r2)= List.hd x_set in (* Un seul noeud du PI-Set 
                    suffit pour déterminer les coups jouables puisque chacun 
                    des noeuds ont les mêmes coups possibles étant donné
                    qu'ils appartiennent au même PI-Set *)
        if b1 + b2 + r1 + r2 = 3 then []        (* partie terminée -> 
                                                    pas de coups possibles *)
        else if b1 + b2 + r1 + r2 <= 2 then (   (* début de partie -> les coups 
                                possibles sont des dispositions de fantômes.*)
        ...
        let rec explore (k_from : int) (acc : alternative list) : alternative list = 
            (*  parcourt récursivement toutes les cases du plateau de manière 
                décroissante pour déterminer tous les coups possibles. *)
            ...
            explore
                ...
                (if not (is_current_ghost b.[k_from]) then 
                    ...
                    List.fold_left (fun acc move -> 
                        ...
                        if on_the_board k_from k_to && 
                            not (is_current_ghost b.[k_to]) then 
                                (k_from, k_to)::acc
                        else acc
    ...
    let convert_to_info_set (x_set : pi_set) : info_set = 
        (* précondition : x_set est non vide et x_set est un ensemble 
            d'informations partiel pour le joueur dont c'est le tour *)
        (* Un info set est simplement un noeud où les fantômes adverses ne sont 
            pas forcément de la couleur renseignée *)
        List.hd x_set

    let convert_to_pi_set ((p, _, b1, r1, b2, r2) as i : info_set) : pi_set = 
        (* sortie : renvoie la liste des noeuds que contient l'ensemble 
            d'information représenté par i 
        Remarque : en configuration 4x4, il ne peut y avoir deux fois le même 
            noeud dans le pi_set renvoyé puisque le jeu s'arrête une fois qu'il 
            y a moins de 4 fantômes sur le plateau, mais en configuration 
            supérieure il peut y avoir énormémement de redondance. Mais 
            celles-ci rendent compte de la probabilité d'avoir tel ou tel 
            fantôme à tel endroit si l'on considère que la disposition des 
            fantômes initiale est équiprobable. 
        Remarque 2 : cette fonction ne fonctionne pas correctement si un 
            fantôme inconnu est mort (donc pas de problème en Configuration4x4, 
            à part pour les états finaux) puisque Placement.put_placement ne 
            fonctionne pas correctement si un fantôme inconnu est mort *)
        List.map 
    ...
    let pi_set_player (x_pi_set : pi_set) : player = 
        ...
        match x_pi_set with 
        | [] -> failwith "Impossible de déterminer le joueur d'un ensemble 
            d'information partielle vide."
        | (p, _, _, _, _, _)::_ -> p
    ...
    let is_leaf ((p, b, b1, r1, b2, r2) : node) : bool = 
        (*  entrée : un noeud
            sortie : true ssi le noeud est une feuille, i.e la partie est 
                terminée dans ce noeud.  *)

        List.exists (fun k -> 
            match Tools.read_char b.[k] with 
            | Some P1, B, _ -> true | _ -> false) Config.p1_exit_cases || 
        List.exists (fun k -> 
            match Tools.read_char b.[k] with 
            | Some P2, B, _ -> true | _ -> false) Config.p2_exit_cases || 
        (b1 = 0 && r1 <> 0) || 
        (r1 = 0 && b1 <> 0) || 
        (b2 = 0 && r2 <> 0) || 
        (r2 = 0 && b2 <> 0) (* && ... pour prendre en compte la racine et ses 
                                enfants (= les noeuds de placement)*)
    ...
    let payoff (p : player) ((_, b, b1, r1, b2, r2) : node) : float = 
        ...
        (* On désambiguise une règle : si un fantôme bleu sort en mangeant un 
        fantôme rouge, le fantôme bleu gagne. *)
        if List.exists (fun k -> 
            match Tools.read_char b.[k] with 
            | Some P1, B, _ -> true | _ -> false) Config.p1_exit_cases  then 
            if p = P1 then 1. else -1.
        else if List.exists (fun k -> 
            match Tools.read_char b.[k] with 
            | Some P2, B, _ -> true | _ -> false) Config.p2_exit_cases then
            if p = P1 then -1. else 1.
    ...
    let opg_children (x_set : (node * strategy) list) (g : game) : 
        (set * alternative) list = 
        (* One player game children 
        pré-condition : x_set est non vide, ne contient pas deux noeuds 
            identiques
        sortie : la liste des enfants de l'ensemble x_set. Un enfant est un 
            ensemble d'ensembles à informations imparfaites directement en 
            dessous de x_set dans l'arbre à un joueur via une coup précis.*)

        (* Etape 1 : on génère les pi_sets découlant des alternatives possibles 
        pour le joueur à qui c'est le tour *)
        let pi_sets_games_alts = List.map (fun alt -> 
            ((List.map (fun (n, str) -> descend alt n, str) x_set),alt::g, alt)
            ) (alternatives (Tools.keys x_set)) in 

        (* Etape 2 : on transforme ces pi_sets en information imparfaite pour 
            le joueur adverse *)
        (* Rien à faire : chacun des noeuds d'un pi_set de pi_sets_alts 
        représente déjà un ensemble d'information différent 
        pour le joueur adverse. En effet, soit un ensemble d'information 
        partiel x de pi_sets_alts. On sait qu'il n'y pas deux noeuds identiques 
        dans x (précondition). Donc toutes les noeuds représentent des 
        dispositions de fantômes de l'adversaire différentes. Donc les noeuds 
        de x représentent chacun un ensemble d'information différent pour 
        l'adversaire. *)

        (* Etape 3 : on récupère chacun des ensembles (qui ne sont plus des 
        ensembles d'information) découlant de l'alternative choisie par le 
        joueur adverse sous la stratégie pi *)
                    ...
                    (Set (List.map 
                        (fun (n, str) -> 
                            if is_leaf n then 
                                ...
                                let opp_alt = str g (convert_to_info_set [n]) 
                                in descend opp_alt n, str, opp_alt::g ) 
                        pi_set
                        ), alt)::acc
                ) [] pi_sets_games_alts

    let tpg_children (x_set : (node * strategy) list) (g : game) : 
            (set * alternative) list = 
        (* Two player game children 
        pré-condition : x_set est non vide, ne contient pas deux noeuds 
            identiques
        post-condition : la liste des enfants de l'ensemble x_set. Un enfant 
            est un ensemble d'ensembles à informations imparfaites directement 
            en dessous de x_set dans l'arbre à deux joueurs via un coup précis.
            *)

        (* Etape 1 : on génère les ensembles de noeuds découlant des 
        alternatives possibles pour le joueur à qui c'est le tour *)
        List.map (fun alt -> 
            (Set (List.map (fun (n, str) -> descend alt n, str, alt::g) x_set), 
            alt)) (alternatives (Tools.keys x_set))
...
module Encoding = struct 
    let encode ((p, b, b1, r1, b2, r2) as n : info_set) : int = 
        ...
        sortie : on code sur 22 bits l'ensemble d'information qui représente un 
            état du jeu : 
            - 4 premiers bits = position (indice k) fantôme bleu connu
            - 4 bits suivants = position fantôme rouge connu 
            - 4 bits suivant : position du fantôme inconnu provenant de la case 
                de départ n°0
            - 4 bits suivant : position du fantôme inconnu provenant de la case 
                de départ n°1
            - bit suivant : b1
        ...
        let u1_pos = ref 0 in 
        let blue_num = ref 0 in (* num de la case de provenance du fantôme bleu 
            connu *)
        let unknown_origin = ref (-1) in (* Variable utile seulement s'il n'y a 
            qu'un seul fantôme inconnu de vivant. 0 s'il provient de 0, 1 
            sinon *)
        ...
        (* S'il n'y a qu'un seul fantôme inconnu de vivant, il faut préciser si 
            c'est u0 ou u1, pour cela on modifie u_pos du fantôme mort de
            manière à ce que u0_pos + u1_pos donne un nombre pair si c'est le 
            fantôme u0 qui est toujours vivant et impair si c'est u1 *)
        if (match p with P1 -> b2 + r2 = 1 | P2 -> b1 + r1 = 1) then 
            if !unknown_origin = 0 then (* u0_pos + u1_pos = pair *)
                u1_pos := !u0_pos mod 2             
            else if !unknown_origin = 1 then (* u0_pos + u1_pos = impair *) 
                u0_pos := 1 + (!u1_pos mod 2)   
            else begin 
                Printf.printf "Attention : voici le noeud qui provoque une 
                    erreur :\n";
                Tools.print_node n;
                failwith "Erreur lors de l'encodage : le seul fantôme inconnu 
                    de vivant n'a pas été repéré.\n" end;
        !b_pos + 
    ...
    let decode (code : int) : info_set option = 
        (* précondition : code >= 0 et on est config4x4 et il existe un unique
            info_set n tel que encode n = code.
        sortie : n *)
        ...
        | 0 ->
            P1,
            (* Plusieurs possibilités pour le joueur 2 inconnu : soit il a deux 
                fantômes, soit il n'en a qu'un seul (ou 0 mais ça ne change 
                rien) auquel cas il faut déterminer si c'est u0 ou u1 *)
            (if b2 + r2 = 2 then 
                String.init Config.board_length (fun k ->  
                    ...
                    if k = u0_pos then Tools.char P2 B 0 else       
                    if k = u1_pos then Tools.char P2 R 1            
                    else '.' 
                    (* les couleurs des fantômes adverses renseignées n'ont *)
                    (* aucune importance puisqu'il s'agit d'un info_set *))
            else if b2 + r2  = 0 then
                String.init Config.board_length (fun k ->  
                    if k = b_pos && b1 = 1 then Tools.char P1 B blue_num else 
                    if k = r_pos  && r1 = 1 then Tools.char P1 R (1-blue_num)
                    else '.')
            else if (u0_pos + u1_pos) mod 2 = 0 then 
                (* Alors c'est le fantôme u0 qui est toujours vivant. *)
                String.init Config.board_length (fun k ->  
                    if k = b_pos && b1 = 1 then Tools.char P1 B blue_num else 
                    if k = r_pos  && r1 = 1 then Tools.char P1 R (1-blue_num) 
                    else  
        ...
        | 1 -> 
            ...
            (if b1 + r1 = 2 then 
                String.init Config.board_length (fun k ->  
                    ...
                    if k = u0_pos then Tools.char P1 B 0 else
    ...
    let compare_code ((p, b, b1, r1, b2, r2) : node) (decoded_n: node option) : 
            bool = 
        (* sortie : true ssi decoded_n correspond à (p, b, b1, r1, b2, r2) si 
            l'on ne considère que les fantômes connus.
        Cette fonction sert à vérifier que code et decode fonctionnent 
        correctement. *)
        ...
        | Some (p', b', b1', r1', b2', r2') -> begin
            ...
            && List.for_all (fun k -> 
                ...
                | _ -> b.[k] = b'.[k]) 
                (List.init Config.board_length (fun k -> k))
        end

    let rec test_code (n : node) (cnt : int) (max_cnt : int) 
                        (history : alternative list): unit = 
        (* test récursvivement les fonctions code et décodent sur tous les 
            descendants de n jusqu'à la profondeur max_cnt - cnt. *)
        if not (compare_code n (decode (encode n))) then (
            Printf.printf "L'encodage perd de l'information pour le noeud 
                suivant : \n";
            Printf.printf "n : \n";
            Tools.print_node n;
            ...
            | Some invented_n ->begin 
                ...
                | None -> Printf.printf "encode (decode (encode noeud_inventé))
                    : None \n"
                | Some invented_invented_n -> begin 
                    Printf.printf 
                        "encode (decode (encode noeud_inventé)) : %d \n" 
                        (encode invented_invented_n);
                    Printf.printf "history : ";
                    Tools.print_list history (fun (k_from, k_to) -> 
                        Printf.printf "(%d, %d)" k_from k_to);
                    print_newline ();
                    assert (encode (invented_invented_n) = encode (invented_n)) 
                    end 
                end;
            print_newline();
        );
        if cnt < max_cnt then (
            ...
            List.iter (fun alt -> test_code (GameTree.descend alt n) (cnt+1)
                        max_cnt (alt::history)) alts
        )
...
module Strategy = struct 
    let random_alt (alts : alternative list) (g: game) (i : info_set) 
        (seed : int) : alternative = 
        (* sortie : un élt de alts choisi par une fonction inconnue 
            (Hashtbl.hash ici) fonction de g et seed. 
        Pour alts, g et seed fixés, random_alt alts g seed donne donc toujours 
            le même résultat. *)
    ...
    let create_random_strategy () : strategy = 
        ...
        fun (g : game) (i : info_set) -> 
            ...
            | None -> failwith "L'info set donné en argument à la random 
                strategy n'est pas valide."
            | Some i ->
                random_alt (GameTree.alternatives [i]) g i seed

    let create_strategy (h : ((game * int), alternative) Hashtbl.t) 
            (last_str : strategy) : strategy = 
        (* sortie : créer une stratégie qui renvoie le coup de h si l'état 
            (g,i) est 'dans' h et renvoie le coup de last_str sinon *)
        fun (g : game) (i : info_set) -> 
        match Hashtbl.find_opt h (g, Encoding.encode i) with 
        | None -> last_str g i
        | Some alt -> alt

    let random_str (strs : strategy list) : strategy = 
        (* sortie : un stratégie tirée uniformément dans la liste des 
            stratégies donnée *)
        let n = List.length strs in 
        let random_index = Random.int n in 
        List.nth strs random_index
end 
...
module UserInterface = struct 
    ...
    let rec user_alternative ((p, b, _, _, _, _) as n : node) : alternative = 
        (* demande à l'utilisateur de rentrer un coup, puis renvoie ce coup 
            s'il est correct, sinon elle s'appelle récursivement. *)
        ...
        let rec translate (ans : char list) (n : int) : int list =
            (* sortie : Liste de taille n des entiers correspondant aux 
                charactères de la liste.
                La fonction saute 'x'.
            ...
            else if n mod 2 = 1 && 48 < code && code <= 48+Config.board_height 
                then
        ...
        try 
            if Placement.is_placement_node n then begin 
                Printf.printf 
                    "Quelle disposition de fantômes choisissez-vous ?\n";
                for j = 0 to 
                    Array.length Placement.init_possible_placements_array -1 do 
                    Printf.printf "Numéro %d : \n" (j + 1);
                    Placement.print_placement n 
                        Placement.init_possible_placements_array.(j);
                done;
                let ans = 
                    try read_int () 
                    with Failure _ -> 
                        failwith "Veuillez renseigner un chiffre.\n" in 
                if 0 < ans && ans <= Array.length 
                    Placement.init_possible_placements_array then 
                    (-ans, 0)
                else 
                    failwith "Ce chiffre ne correspond à aucune des dispostions 
                                possibles.\n"
            end else begin
    ...
    let rec user_against_pure_strategy (pi : strategy) (user_player : player) : unit = 
        ...
        with IncorrectAnswer -> 
                ...
                user_against_pure_strategy pi user_player
...
module IMP_MINIMAX = struct
    ...
    let partition (x_set : set) : set list * set list = 
        (*  entrée : x un ensemble de noeuds 
            sortie : un ensemble de pi_set, un ensemble de feuilles. 
                ...
                vaut l'ensemble x_set. Les pi_sets appartiennent 
                à des ensembles d'information deux à deux disjoints. *)
    ...
    let rec value (p : player) (x_set : set) (depth : int) 
            (h : (game * int, alternative) Hashtbl.t) (max_depth : int) 
            (nb_opp_strs : int) : float = 
        match x_set with 
        | PI_Set (x_pi_set,g) -> 
            let code = Encoding.encode 
                (GameTree.convert_to_info_set (Tools.keys x_pi_set)) in
            
            if depth >= max_depth then 
                0. (* puisque que l est un ensemble d'information partielle, 
            ça ne peut pas être une feuille donc inutile d'appeler payoff *)
            else begin
            (* max {V(extend(Y)) | Y is a child of X } + on met la meilleure 
            alternative dans la stratégie *)
            let rec max_value (l : (set*alternative) list) (maxi : float)  
                    (maxi_alt : alternative) : float= 
                match l with 
                | [] ->  
                    Hashtbl.add h (g, code) maxi_alt;
                    maxi
                | (y_set, alt) :: l' -> 
                    let score = value p y_set (depth+1) h max_depth nb_opp_strs 
                    in 
                    if score > (maxi +. epsilon) then max_value l' score alt
                    else if score > (maxi -. epsilon) && Random.bool () then 
                    (*maxi et score sont égaux alors on choisit aléatoirement*)
                        max_value l' score alt 
                    else max_value l' maxi maxi_alt in
            
            let chiildren = 
                Tools.shuffle_list (GameTree.opg_children x_pi_set g) in 
        ...
        | Set l -> 
            ...
            List.fold_left (fun sum sset -> 
                ...
                | Leaf (leaf, str, g) ->
                    let score = (1. /. (float nb_opp_strs)) 
                        *. (GameTree.payoff p leaf)  in 
                    sum +. score
                    ) 0. leaves 
            +.
            List.fold_left (fun sum x -> 
                let score= value p x depth h max_depth nb_opp_strs in 
                sum +. score) 0. pi_sets
        | Leaf (leaf,str, g) -> (1. /. (float nb_opp_strs)) 
                                    *. (GameTree.payoff p leaf)


    let imp_minimax (previous_opp_strs : strategy Queue.t) (p : player) 
        (my_last_str : strategy) (max_depth : int): float * strategy =
        let h = Hashtbl.create Tools.power_of_2.(10) in 
        let score = 
            match p with 
            | P1 -> 
                value P1 (Set (Queue.fold 
                    (fun acc opp_str -> (GameTree.root, opp_str, [])::acc) 
                    [] previous_opp_strs)) 
                    0 h max_depth (Queue.length previous_opp_strs)
            | P2 ->
                value P2 (Set (Queue.fold 
                    (fun acc opp_str -> 
                        let p1_alt = opp_str [] 
                            (GameTree.convert_to_info_set [GameTree.root]) 
                        in ((GameTree.descend p1_alt GameTree.root), 
                        opp_str, [p1_alt])::acc) 
                    [] previous_opp_strs)) 
                    0 h max_depth (Queue.length previous_opp_strs)
        in 
    ...
    let imp_alpha_beta (previous_opp_str : strategy list) (p : player) 
            (my_last_str : strategy) (max_depth : int): float * strategy = 
        let h = Hashtbl.create Tools.power_of_2.(10) in 
        let nb_strs = List.length previous_opp_str in 
        let prob (x_set : set) : float = 
            match x_set with
            | Set l -> (float (List.length l)) /. (float nb_strs)
            | PI_Set (l,g) -> (float (List.length l)) /. (float nb_strs)
            | Leaf (n, str, g) -> 1. /. (float nb_strs) in 
        
        let rec max_set ((x,g) : (node * strategy) list * game) (ppp : float) 
            (depth : int) : float = 
            try 
                let best = ref ppp in 
                (* debut ajout *)
                if depth >= max_depth then max 0. !best else begin
                let maxi = ref (-100.0) in (* sert à déterminer maxi_alt, 
                                                    maxi <= best *)
                ...
                List.iter (fun (child, a) ->
                    ...
                    if temp > !best then (
                        if temp = 1. *. (prob child) then (
                            ...
                            Hashtbl.add h 
                                (g, Encoding.encode 
                                (GameTree.convert_to_info_set (Tools.keys x))) 
                                a;
                            (* fin ajout *)
                            raise (Return temp)
                        );
                ...
                ) (Tools.shuffle_list (GameTree.opg_children x g)); 
                                                                (* aléatoire *)
                (* debut ajout *)
        ...
        and mixed_set (x_set : set) (ppp : float) (depth : int): float = 
            try 
                ...
                List.iter (fun x -> 
                    ...
                    | PI_Set (x, g) ->
                        sum := !sum +. 
                        max_set (x,g) (ppp -. !sum -. (1. *. !prob_left)) depth 
                    end;
        ...
        let score = 
            match p with 
            | P1 -> 
                mixed_set (Set (List.map 
                        (fun opp_str -> (GameTree.root, opp_str, [])) 
                        previous_opp_str)) (-100.) 0
    ...
    let rec tpg_value (p : player) (x_set : set) (depth : int) (h : (game * int, alternative) Hashtbl.t) (max_depth : int) : float = 
        ...
        begin 
            ...
            match GameTree.pi_set_player (Tools.keys x_pi_set) with 
            | p' when p = p' -> 
                ...
                let rec max_value (l : (set*alternative) list) (maxi : float)  (maxi_alt : alternative) : float= 
                    match l with 
                    | [] ->  
                        Hashtbl.add h (g, Encoding.encode (GameTree.convert_to_info_set (Tools.keys x_pi_set))) maxi_alt;
                        maxi
    ...
    let pure_tpg_imp_minimax (p : player) (depth : int): strategy =
        (* Peut être utilisée comme stratégie initiale dans la méthode 
        Fictitious Play. Cet algorithme n'a aucune raison d'être correct. *)
        let h = Hashtbl.create Tools.power_of_2.(10) in 
        let str = fun g i -> (-42, -42) in (* la stratégie adverse ne sert à rien dans tpg_imp_minimax *)
        fun (g : game) (i : info_set) -> 
            let code = Encoding.encode i in
            match Hashtbl.find_opt h (g, code) with 
            | Some alt -> alt 
            | None -> let _ = tpg_value p (Set (List.map (fun n -> n,str, g) (GameTree.convert_to_pi_set i))) 0 h (depth - (List.length g / 2)) in 
                Hashtbl.find h (g, code)

    
    let fictitious_play (nb_rounds : int) (depth : int) : 
        ...
        let last_p1_str = ref (Strategy.create_random_strategy ()) in 
        let last_p2_str = ref (Strategy.create_random_strategy ()) in
        ...
        for round = 1 to nb_rounds do 
            let score1, new_p1_str = 
                imp_minimax previous_p2_strs P1 !last_p1_str depth in 
            last_p1_str := new_p1_str;
            Queue.push new_p1_str previous_p1_strs;
            if Queue.length previous_p1_strs > 200 then
            ...
            let score2, new_p2_str = 
                imp_minimax previous_p1_strs P2 !last_p2_str depth in 
            last_p2_str := new_p2_str;
            Queue.push new_p2_str previous_p2_strs;
            if Queue.length previous_p2_strs > 200 then 
        ...
        Queue.fold (fun acc str -> str::acc) [] previous_p1_strs, 
        Queue.fold (fun acc str -> str::acc) [] previous_p2_strs

     let fictitious_play_alpha_beta (nb_rounds : int) (depth : int) : 
            strategy list * strategy list = 
        Printf.printf "Création des stratégies aléatoires initiales.\n";
        let previous_p1_strs = ref [Strategy.create_random_strategy ()] in 
        let previous_p2_strs = ref [Strategy.create_random_strategy ()] in 

        Printf.printf "Stratégies aléatoires initiales créées.\n";
        for round = 1 to nb_rounds do 
            let score1, new_p1_str = 
                imp_alpha_beta !previous_p2_strs P1 (List.hd !previous_p1_strs) 
                                depth in 
            previous_p1_strs := new_p1_str::!previous_p1_strs;
            ...
            let score2, new_p2_str = 
                imp_alpha_beta !previous_p1_strs P2 (List.hd !previous_p2_strs) 
                                depth in 
            previous_p2_strs := new_p2_str::!previous_p2_strs;

            Printf.printf "Tour %d prof %d : P2 répond à P1 avec %f.\n" 
                            round depth score2;
            flush stdout;
        done;
        !previous_p1_strs, !previous_p2_strs
