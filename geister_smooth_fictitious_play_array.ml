(* Dans ce fichier, l'on a implémenté la méthode smooth ficitious play 
en représentant toutes les structures de données par des tableaux au lieu 
de listes pour gagner de la rapidité. Ca n'a absolument rien changé : le 
programme n'est ni plus rapide, ni plus lent.*)
...
type pi_set = (node * float) array                             (* pi_set : partial information set. 
...
type strategy = game -> info_set -> float array     (* Pour jouer un coup, il faut connaitre les coups précédents (game) et l'état actuel du jeu (info_set).
                                                                    La partie seule n'est pas suffisante car les premiers coups ne renseigent pas sur la disposition des fantômes afin 
                                                                    que celle-ci reste cachée à l'adversaire.
                                                                    Représente une stratégie *)

type ghost = B | R  | EmptyCase                     (* Représente un fantôme. *)

type set =  | Set of ((node * float * game) array) array             (* Représente un ensemble de noeuds (états de la partie). *)
...
module Config = struct
    ...
    let configuration_3x4 = (4, 3, 1, 1, [9;10], [1;2], [0;3], [8; 11], [4; 1; -4; -1])
    let configuration_3x3 = (3, 3, 1, 1, [6;7], [1;2], [0], [8], [3; 1; -3; -1])
...
module Tools = struct 
    ...
    let print_game (game : game) = 
        print_list game (fun (k_from, k_to)-> Printf.printf "(%d, %d)" k_from k_to)
...
module GameTree = struct 
    ...
    let alternatives (x_set : pi_set) : alternative array = 
        (* sortie : la liste des coups possibles depuis un ensemble d'information partielle.*)

        let (p, b, b1, r1, b2, r2)= fst (x_set.(0)) in (* Un seul noeud du PI-Set suffit 
        ...
        if b1 + b2 + r1 + r2 = 3 then [||]        (* partie terminée -> pas de coups possibles *)
            ...
            Array.init (Placement.number_of_init_possible_placements) (fun i -> (-i-1, 0))
        ) else
        ...
        Array.of_list (explore (Config.board_length-1) [])
    ...
    let pi_set_player (x_pi_set : pi_set) : player = 
        ...
        let ((p, _, _, _, _, _), _) = x_pi_set.(0) in p
    ...
    let opg_children (opp_strs : strategy Queue.t) (x_set : pi_set) (g : game) : (set * alternative) array = 
        ...
        let pi_sets_games_alts = Array.map (fun alt -> ((Array.map (fun (n, w) -> descend alt n, w) x_set),
        ...
        Array.map (fun (pi_set, g, alt) -> 
                    (Set (Array.map  
                        (fun (n, w) -> 
                            if is_leaf n then 
                                [|(n, w, g)|]
                            else
                                let alts = alternatives [|(n, w)|] in 
                                (Array.map (fun i -> 
                                    let opp_alt = alts.(i) in 
                                    descend opp_alt n, 
                                    w *. (Queue.fold (fun acc str -> (str g n).(i) +. acc) 0. opp_strs) /. (float (Queue.length opp_strs)),
                                    if fst opp_alt < 0 then (0, -1)::g else opp_alt::g) (Array.init (Array.length alts) (fun i -> i))) ) 
                        pi_set
                        ), alt)
                ) pi_sets_games_alts

    (* let tpg_children (x_set : (node * strategy) list) (g : game) : (set * alternative) list = 
        ...
        List.map (fun alt -> (Set (List.map (fun (n, str) -> descend alt n, str, alt::g) x_set), alt)) (alternatives (Tools.keys x_set)) *)
...
module Strategy = struct 
    let create_uniform_strategy () : strategy = 
        ...
        Array.map (fun alt -> 1. /. (float n)) alts

    let create_strategy (h : ((game * int), float array) Hashtbl.t) (last_str : strategy) : strategy = 
...
module UserInterface = struct 
    ...
    exception Alternative of alternative
    let pick_alternative (alts : alternative array) (alt_probs : float array) : alternative = 
        (* précondition : Array.length alts = Array.length alt_probs *)
        let r = ref (Random.float 1.) in 
        try 
            for i = 0 to Array.length alts -1 do 
                if !r <= alt_probs.(i) then 
                    raise (Alternative alts.(i))
                else
                    r := !r -. alt_probs.(i)
            done;
            Printf.printf "Pick_alternative : r = %f\n" !r; 
            failwith "alt_probs n'est pas correcte."
        with Alternative alt -> alt
            ...
            if GameTree.is_leaf n then (
                ...
                let alt = 
                    if p = user_player then (
                        ...
                        print_newline (); *)
                        pick_alternative alts alt_probs in 
    ...
    let strategy_against_strategy (p1_str : strategy) (p2_str : strategy) (max_cnt : int): player option = 
        ...
        let rec play_game ((p, b, b1, r1, b2, r2) as n : node) (cnt : int) (g: game): player option= 
            if GameTree.is_leaf n || cnt > max_cnt then (
                ...
                let alt = pick_alternative alts alt_probs in 
...
module IMP_MINIMAX = struct
    ...
    let partition (x_set : set) : set list * set list = 
        ...
        | Set a       -> 
            ...
            Array.iter (fun (b : (node * float * game) array) -> 
                Array.iter (
                    fun (n, w, g) -> 
                    let c = Encoding.encode n in 
                    match Hashtbl.find_opt hash_table (g,c) with 
                    | None -> Hashtbl.add hash_table (g,c) [(n, w)]
                    | Some (nodes_lst) -> Hashtbl.replace hash_table (g,c) ((n, w)::nodes_lst)
                ) b
            ) a;
            Hashtbl.fold (fun (g,c) nodes_lst (leaves, pi_sets) -> 
                    let new_leaves, piset = 
                        List.fold_left (fun (leaves, piset) (n, w) -> if GameTree.is_leaf n then (Leaf (n,w,g))::leaves, piset else leaves,(n,w)::piset) 
                                        ([], []) nodes_lst
                    in new_leaves@leaves, match piset with [] -> pi_sets | _ -> (PI_Set (Array.of_list piset, g)::pi_sets)
            ) hash_table ([], [])
    
    let epsilon = 0.05 

    let softmax (alt_scores : float array) (lambda : float): float array = 
        if lambda <= 709.0 then 
            let total = Array.fold_left (fun acc score -> exp (lambda *. score) +. acc) 0. alt_scores in
            Array.map (fun prob -> (exp (lambda *. prob) /. total)) alt_scores
        else begin
            Printf.printf "lambda > 709.0\n";
            let max_score = Array.fold_left (fun best_score score -> max score best_score) neg_infinity alt_scores in 
            let total = Array.fold_left (fun acc score -> (if Tools.compare_scores score max_score 0.001 then 1. else 0.) +. acc) 0. alt_scores in 
            Array.map (fun score -> (if Tools.compare_scores score max_score 0.001 then 1. else 0.) /. total) alt_scores
        end

    let rec value (p : player) (opp_strs : strategy Queue.t) (x_set : set) (depth : int) (h : (game * int, float array) Hashtbl.t) (max_depth : int) (lambda : float): float = 
        match x_set with 
        | PI_Set (x_pi_set,g) -> 
            ...
            let alt_scores = Array.map (fun (y_set, alt) -> value p opp_strs y_set (depth+1) h max_depth lambda) children in
            Hashtbl.add h (g, code) (softmax alt_scores lambda);
            Array.fold_left (fun best_score score -> max best_score score) neg_infinity alt_scores
            end
        | Set l -> 
            ...
            +.
            List.fold_left (fun sum x -> let score= value p opp_strs x depth h max_depth lambda in 
                sum +. score) 0. pi_sets
        | Leaf (leaf,w, g) -> w *. (GameTree.payoff p leaf)

    let imp_minimax (previous_opp_strs : strategy Queue.t) (p : player) (my_last_str : strategy) (max_depth : int) (lambda : float): float * strategy =
        let h = Hashtbl.create Tools.power_of_2.(10) in 
        let score = 
            match p with 
            | P1 -> 
                value P1 previous_opp_strs (Set [|[|(GameTree.root, 1., [])|]|]) 0 h max_depth lambda
            | P2 ->
                let x = Set  (let alts = GameTree.alternatives [|(GameTree.root, 1.)|] in 
                                [|(Array.map (fun i -> 
                                    let opp_alt = alts.(i) in 
                                    GameTree.descend opp_alt GameTree.root, 
                                    (Queue.fold (fun acc str -> (str [] GameTree.root).(i) +. acc) 0. previous_opp_strs) /. (float (Queue.length previous_opp_strs)),
                                    [(0, -1)]) (Array.init (Array.length alts) (fun i -> i))
                                )|]) 
                in 
    ...
    let smooth_fictitious_play (nb_rounds : int) (depth : int) (lambda : float) : strategy * strategy = 
        ...
        Printf.printf "Stratégies aléatoires intiales crées.\n";
        for round = 1 to nb_rounds do 
            let score1, new_p1_str = imp_minimax previous_p2_strs P1 !last_p1_str depth lambda in 
            ...
            let score2, new_p2_str = imp_minimax previous_p1_strs P2 !last_p2_str depth lambda in 
